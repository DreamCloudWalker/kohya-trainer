...
# CLIP获取功能时cutout的使うか：使う場合に牙齿ソース的書き換えてください
NUM_CUTOUTS = 4
USE_CUTOUTS = False
...
class PipelineLike:
    ...
    # region xformers或要使用的零件：独自に書き換える的で関係なし
        ...
        **kwargs,
        ...
        if self.clip_guidance_scale > 0:
            ...
            for i, clip_prompt in enumerate(clip_prompts):
                if clip_prompt is not None:  # clip_prompt如果有
            ...
            text_embeddings_clip = text_embeddings_clip / text_embeddings_clip.norm(p=2, dim=-1, keepdim=True)  # prompt即使在多种情况下OK
        ...
        if (
            ...
            if self.clip_image_guidance_scale > 0:
                ...
                size = (width // VGG16_INPUT_RESIZE_DIV, height // VGG16_INPUT_RESIZE_DIV)  # 目前1/4に（小さいか?）
    ...
    def cond_fn_vgg16_b1(self, latents, timestep, index, text_embeddings, noise_pred_original, guide_embeddings, guidance_scale):
        ...
        # 我不知道它是否正常工作，如果批处理大小为多个
        loss = ((image_embeddings - guide_embeddings) ** 2).mean() * guidance_scale  # MSE style transferでコンテンツ的損失牙齿MSEな的で
...
def preprocess_guide_image(image):
    image = image.resize(FEATURE_EXTRACTOR_SIZE, resample=Image.NEAREST)  # cond_fn结合
...
# VGG16的入力牙齿任意サイズでよい的で入力图像適宜リサイズする
def preprocess_vgg16_guide_image(image, size):
    image = image.resize(size, resample=Image.NEAREST)  # cond_fn结合
    image = np.array(image).astype(np.float32) / 255.0
    image = image[None].transpose(0, 3, 1, 2)  # nchw
    image = torch.from_numpy(image)
    return image  # 0 to 1
...
def main(args):
    ...
    if args.v_parameterization and not args.v2:
        print("v_parameterization should be with v2 / v1でv_parameterization的使用すること牙齿想定されていません")
    if args.v2 and args.clip_skip is not None:
        print("v2 with clip_skip will be unexpected / v2でclip_skip的使用すること牙齿想定されていません")
    ...
    # Extended Textual Inversion 和 Textual Inversion的処理する
    ...
    if args.textual_inversion_embeddings:
        ...
        for embeds_file in args.textual_inversion_embeddings:
            ...
            if type(embeds) != torch.Tensor:
                raise ValueError(f"weight file does not contains Tensor / 重みファイル的データ但Tensorで牙齿ありません: {embeds_file}")
            ...
            ), f"tokenizer has same word to token string (filename). please rename the file / 指定名称（ファイル名）的トークン但既に存在します。ファイル的リネームしてください: {embeds_file}"
    ...
    for gen_iter in range(args.n_iter):
        ...
        while args.interactive or prompt_index < len(prompt_list):
            ...
            for pi in range(args.images_per_prompt if len(raw_prompts) == 1 else len(raw_prompts)):
                ...
                # 使用同一图像时、本当牙齿latentに変換しておくと無駄但ない但面倒な的で目前毎回処理する
                if init_images is not None:
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--v2", action="store_true", help="load Stable Diffusion v2.0 model / Stable Diffusion 2.0的阅读模型")
    parser.add_argument(
        "--v_parameterization", action="store_true", help="enable v-parameterization training / v-parameterization学習的有効是"
    )
    ...
    parser.add_argument(
        ...
        default=None,
        nargs="*",
        help="Embeddings files of Extended Textual Inversion / Extended Textual Inversion的embeddings",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="enable CLIP guided SD, scale for guidance (DDIM, PNDM, LMS samplers only) / CLIP guided SD的有効にしてこ的scale的適用する（サンプラー牙齿DDIM、PNDM、LMS的み）",
    )
    parser.add_argument(
        "--clip_image_guidance_scale",
        type=float,
        default=0.0,
        help="enable CLIP guided SD by image, scale for guidance / 画像によるCLIP guided SD的有効にしてこ的scale的適用する",
    )
    parser.add_argument(
        "--vgg16_guidance_scale",
        type=float,
        default=0.0,
        help="enable VGG16 guided SD by image, scale for guidance / 画像によるVGG16 guided SD的有効にしてこ的scale的適用する",
    )
    parser.add_argument(
        "--vgg16_guidance_layer",
        type=int,
        default=20,
        help="layer of VGG16 to calculate contents guide (1~30, 20 for conv4_2) / VGG16的contents guideに使うレイヤー番号 (1~30、20牙齿conv4_2)",
    )
    parser.add_argument(
